...
try {
    // 1. Verify that category exists in the database
    ...
    // 3. Save product details in database
    $stmt = $conn->prepare("INSERT INTO products (category_id, name, description, price, stock_quantity, image_url) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("issdis", $category_id, $name, $description, $price, $stock_quantity, $image_path);
    ...
    if ($stmt->execute()) {
        ...
        http_response_code(500);
        echo json_encode(["success" => false, "message" => "Failed to save product details."]);
    }
    $stmt->close();
